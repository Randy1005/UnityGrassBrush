Shader "Custom/GrassTest"
{
    Properties
    {
        ...
        _NoiseMap("Density noise texture", 2D) = "white" {}
        ...
        Pass
        {
            ...
            void geom(triangle v2g IN[3] : SV_POSITION, inout TriangleStream<g2f> tristream) {
                ...
                for (uint i = 0; i < grassBlades; i++) {
                    ...
                    float2 noiseSample = tex2Dlod(_NoiseMap, float4(uvOrig, 0.0, 0.0)).xy;
                    ...
                    float heightFactor =  _GrassHeight * noiseSample.x * grassHeightTuningFactor;
                    ...
                    tristream.Append(getGrassVertex(bladeTopPoint, float4(0.5, 1, IN[0].uv), fixed4(1, 0.8, 1, 1)));

                    // append bladePoint2 to triangle stream
                    tristream.Append(getGrassVertex(bladePoint2, float4(1, 0, IN[0].uv), fixed4(0, 0, 0, 1)));
                }
